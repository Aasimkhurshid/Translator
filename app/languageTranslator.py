...
class translator():
    ...
    def translate_frm_file(filepath, lang):
        translated = GoogleTranslator(source='auto', target=lang).translate_file(filepath)
        print(translated)
        return translated
...
def translateFromFile(filesPath, outputPath, targetlang):

    to_translate = 'Ohh this work is giving something to work with'
    translator.translate(to_translate, targetlang)
    ...
    for file_name in srt_files_test:
        ...
        if file_name.endswith(".srt"):
            ...
            tanslated = translator.translate_frm_file(file_name, targetlang)
